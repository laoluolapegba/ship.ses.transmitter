    ...
    CancellationToken cancellationToken = default)
        {
            ...
            _logger.LogInformation("📡 Sending {Method} {Endpoint} for {ResourceType} (id={ResourceId}) via {Route}",
        ...
        public async Task<FhirApiResponse> SendAsync1(
        FhirOperation operation,
        string resourceType,
        string resourceId = null,
        string jsonPayload = null,
    ...
    CancellationToken cancellationToken = default)
        {
            ...
            if (string.Equals(shipService, "PDS", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (string.Equals(normalized, "Bundle", StringComparison.OrdinalIgnoreCase))
                {
                    var bundlePath = route.BaseUrl; // 
                    if (string.IsNullOrWhiteSpace(bundlePath))
                        bundlePath = "/api/v1/Patient/Bundle"; // sensible default

                    endpoint = $"{baseUrl}{(bundlePath!.StartsWith('/') ? "" : "/")}{bundlePath}";
                    method = HttpMethod.Post;
                }
                else
                {
