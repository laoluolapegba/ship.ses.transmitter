...
| 1.2 | `appsettings.json` → `AuthSettings`; `Settings/AuthSettings.cs` | Single outbound FHIR credential block. | One outbound client id + secret for all FHIR calls. | All clients' data transmitted under one SHIP identity. | ✅ **Done (2026-05-30, Phases 2–4):** per-`clientId` resolution via `IClientCredentialProvider`; `VaultClientCredentialProvider` reads `secret/ses/clients/{clientId}/hmac` when `ClientCredentials:Source=Vault`. `AuthSettings` is now non-secret defaults (`TokenEndpoint`/`GrantType`) + the `Config` fallback; `ClientSecret` unused under Vault. | High → resolved |
...
| 1.4 | `Settings/FhirRoutingSettings.cs` + `appsettings.json` → `FhirRouting.Apis[].{Scope, ClientCert, BaseUrl}` | Per-target (`PDS`,`SCR`) routing config including a **single shared** `ClientCert` (`certs/client.pfx`) and per-route `Scope`. | One mutual-TLS cert + scope shared by all clients. | Target endpoints are fine (routing), but the **client certificate is global**, not per-client. | ✅ **Done (2026-05-30):** `ClientCert` removed from `appsettings.json` (Default/PDS/SCR) — it was unused. Kept `BaseUrl`/`Scope`/resource lists as non-secret routing config. Any future mTLS/HMAC secret resolves per-client from Vault. | Medium → resolved (config) |
...
| 1.6 | `Settings/FhirApiSettings.cs` (legacy `FhirApi` block) + `Program.cs:41-70` | Legacy single-endpoint fallback with `ClientCertPath`/`ClientCertPassword`/`CallbackUrlTemplate`. | One base URL + one cert for everything. | Legacy path reinforces single-target/single-cert assumption. | Retain only as routing fallback; strip cert/secret fields, resolve those per client. | Low |
...
| 2.5 | `ReadServices/FhirApiService.cs` | `scope = route.Scope ?? AuthSettings.Scope`; token from global creds, no clientId. | Token derived from global creds + route scope. | Outbound auth was global. | ✅ **FIXED (2026-05-30, Phase 3):** `token = _fhirTokenService.GetAccessTokenAsync(clientId, scope)` — scope stays route-derived (routing), credential is client-derived. | High → resolved |
...
| 3.4 | `Persistance/.../MongoSyncRepository.cs:63` `GetByStatusAsync` (default `take=100`) | `ProcessPendingRecordsAsync` calls it with **no paging args** → fixed 100, no ordering, no per-client cap. | One client → fairness irrelevant. | A high-volume client can monopolize each batch and **starve** other clients. | Add per-client fair batching (round-robin or per-client quotas); deterministic ordering. | Medium |
| 3.5 | `FhirSyncRecord.RetryCount` (Domain) + `MongoSyncRepository.BulkUpdateStatusAsync` | `RetryCount` was set to `0` on ingest and **never incremented**. `BulkUpdateStatusAsync` set status/error/txn/timeSynced/lastAttemptAt but not retryCount. | n/a (latent bug) | Retries not tracked; attempts not observable/boundable. | ✅ **Partial (2026-05-30):** `BulkUpdateStatusAsync` now `Inc`s `RetryCount` on failed updates (`LastAttemptAt` was already set). **Deferred:** true max-retry / dead-letter requeue (Failed→Pending with backoff up to N) — note that `Failed` is currently terminal (not re-fetched as `Pending`), so there is no infinite-retry loop. | Medium |
...
| 4.2 | `FhirApiService.SendAsync` | Token + scope were global; same bearer reused for every client/target. | One token for all clients/targets. | All clients shared one outbound identity. | ✅ **FIXED (2026-05-30, Phase 3):** token resolved by `clientId` via `IFhirTokenService` (cached per `(clientId, scope)`); same client credential across PDS/SCR per design #1, scope still route-derived. | High → resolved |
| 4.3 | `Installers/WorkerServiceExtensions.cs:135-152` (`AddFhirApiClient`) | One named `HttpClient "FhirApi"` with `Default.BaseUrl` + single timeout; **client certificate from config was never applied** to the handler. | One HTTP client / one cert for all targets+clients. | mTLS cert config was dead (not wired); per-client/per-target cert impossible. | ✅ **Partial (2026-05-30):** dead `ClientCert` removed from `appsettings.json`. If mTLS is later required, wire a handler that selects the **per-client** cert from Vault. (C# `ClientCert`/`FhirClientCertificateSettings` types retained for the legacy `FhirApi` binder in `Program.cs`.) | Medium |
