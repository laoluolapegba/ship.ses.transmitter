...
- [x] **Vault: load all clients once at startup** (`IClientCredentialProvider.InitializeAsync`,
      ...
      **listing** the prefix and reads each secret once into memory; `IVaultSecretReader` gains
      ...
      Each client whose secret is present is loaded. Mirrors the Ingestor's
      `VaultClientHmacCredentialLoader`/registry. *(2026-06-01; active/revoked filtering removed 2026-06-02)*
...
- [x] **Vault config via OS env vars (Ingestor-identical), Vault-only** *(2026-06-02 — supersedes the
      ...
      `VAULT_TOKEN`, `VAULT_*` (`VaultClientSecretSettings.FromEnvironment()`); `VAULT_ADDR`/`VAULT_TOKEN`
      required → **worker exits at startup** if unset. Env var names have no `HMAC` segment
